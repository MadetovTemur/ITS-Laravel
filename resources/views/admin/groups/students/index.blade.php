<x-user-layout>
	...
	<x-panel class="flex flex-col  pt-16 pb-16 ">
		<x-splade-lazy class="flex flex-col items-center">
			...
			<Link href="{{ route('admin.groups.students.create', request()->route('group')->id) }}" class="inline w-fit rounded-md shadow-sm bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 focus:outline-none focus:shadow-outline" >
				Add Students
			</Link>

			<x-splade-table class="w-full mt-5"  :for="$groupStudents" striped pagination-scroll="head" >
		    <x-slot:empty-state>
		        <p>Whoops!</p>
		    </x-slot>
